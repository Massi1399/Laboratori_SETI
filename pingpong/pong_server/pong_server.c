...
void tcp_pong(int message_no, size_t message_size, FILE *in_stream, int out_socket)
{
	...
	for (n_msg = 1; n_msg <= message_no; ++n_msg)
	{
		...
		if (clock_gettime(CLOCK_TYPE, &time2) < 0)
			fail_errno("errore nella clock_gettime time2");
		...
		if (clock_gettime(CLOCK_TYPE, &time3) < 0)
			fail_errno("errore nella clock_gettime time3");
...
//andava controllata la fork
	pid = fork();
	if(pid < 0)	
		fail_errno("errore nella fork");
...
//avendo specificato il flag AI-PASSIVE serve inserire NULL come parametro per fare in modo che il socket generato possa accettare connesioni
	gai_rv = getaddrinfo(NULL, argv[1], &gai_hints, &server_addrinfo);

	if (gai_rv != 0)
		fail("errore nella getaddressinfo");

	server_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol);

	if (server_socket < 0)
		fail_errno("errore nella socket");

	if(bind(server_socket, server_addrinfo->ai_addr,server_addrinfo->ai_addrlen)< 0)
		fail_errno("errore nella bind");

//LISTENBACKLOG è definito nell'header come uguale a 1 e serve per indicare fino a quanto può crescere la coda delle richieste in attesa
	if(listen(server_socket, LISTENBACKLOG)<0)
		fail_errno("errore nella listen");
