...
/*** TO BE DONE START ***/
	...
	if( sprintf(message, "%d\n", msg_no) < 0)
		fail("errore nella sprintf di do_ping");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &send_time) < 0)
		fail_errno("errore nella clock_gettime send_time");
...
/*** TO BE DONE START ***/
	...
	if(sent_bytes < 0)
		fail_errno("TCP: Errore nell'inviare il messaggio");
	//manca il controllo sulla quantità di dati inviati
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &recv_time) < 0)
		fail_errno("errore nella clock_gettime recv_time");
...
/*** TO BE DONE START ***/
    ...
    if( write(tcp_socket, request , strlen(request) ) < 0)
    	fail_errno("TCP Ping could not send request");
