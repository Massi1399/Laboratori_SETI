...
double do_ping(size_t msg_size, int msg_no, char message[msg_size], int ping_socket, double timeout)
{
	...
	if (sprintf(message, "%d", msg_no) < 0)
		fail("UDP:errore nella sprintf di do_ping");
	...
	do
	{
		...
		if (clock_gettime(CLOCK_TYPE, &send_time) < 0)
			fail_errno("UDP:errore nella clock_gettime send_time");
...
//superflua per via del flag O_NONBLOCK ?
		sent_bytes = nonblocking_write_all(ping_socket, message, msg_size);
		if (sent_bytes < 0)
			fail("UDP: Errore nell'inviare il messaggio");
	//manca il controllo sui byte inviati
	//usiamo la fail
...
//solo read in quanto con la read_all rimaniamo in attesa inutile
		...
		if (clock_gettime(CLOCK_TYPE, &recv_time) < 0)
			fail_errno("UDP:errore nella clock_gettime recv_time");
...
//rende non bloccanti le operazioni di I/O
//quindi anziché bloccarsi le read/write ritorneranno EAGAIN o EWOULDBLOCK 
	if (fcntl(ping_socket, F_SETFL, O_NONBLOCK) < 0)
		fail_errno("UDP:errore fcntl");
	...
	if (gai_rv != 0)
		fail("UDP:errore nella getaddressinfo");
	...
	if (connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) < 0)
		fail_errno("UDP:errore nella connect");
...
int main(int argc, char *argv[])
{
	...
	gai_hints.ai_family = AF_INET;			 /* Allow IPv4 or IPv6 */
	gai_hints.ai_socktype = SOCK_STREAM; /* Stream socket */
	gai_hints.ai_protocol = 0;					 /* Any protocol */
	...
	if (ask_socket < 0)
		fail_errno("UDP:errore nella socket");

	if (connect(ask_socket, (struct sockaddr *)ipv4, INET_ADDRSTRLEN) < 0)
		fail_errno("UDP:errore nella connect");
