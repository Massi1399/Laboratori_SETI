...
time_t my_timegm(struct tm *tm)
{
	...
	if( pthread_mutex_lock(&my_timegm_mutex) != 0 )
		fail("In my_timegm function,  mutex_lock() error");
	//ritorna il valore della variabile d'ambiente TZ
	//manca un allocazione (con strdup)
	tz = getenv("TZ");

	/*** TO BE DONE 6.0 END ***/
//imposta la variabile d'ambiente TZ, con valore ""	
	setenv("TZ", "", 1);
//inizializza la variabile tzname con una timezone, se la variabile d'ambiente TZ è vuota viene utilizzato UTC		
	tzset();
	ret = mktime(tm);
	if (tz)
	{
	...
	if( pthread_mutex_unlock(&my_timegm_mutex) != 0 )
		fail("In my_timegm function,  mutex_unlock() error");
	/*** TO BE DONE 6.0 END ***/
