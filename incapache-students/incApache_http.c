...
/*** TO BE DONE 6.0 START ***/
		if( pthread_mutex_lock(&cookie_mutex) != 0)
			fail("In get_new_UID function,  mutex_lock() error:");
	
	//prima incrementa CurUID poi effettua operazione modulo MAX_COOKIES	
		retval = ++CurUID % MAX_COOKIES;
	//il modulo andava applicata anche a curID	
		UserTracker[retval] = 0;
		
		if( pthread_mutex_unlock(&cookie_mutex) != 0)
			fail("In get_new_UID function, mutex_unlock() error:");
...
/*** TO BE DONE 6.0 START ***/
		...
		if( pthread_mutex_lock(&cookie_mutex) != 0 )
			fail("In keep_track_of_UID function, mutex_lock() error:");
		
		newcount = ++UserTracker[myUID] ;

		if( pthread_mutex_unlock(&cookie_mutex) != 0)
			fail("In keep_track_of_UID function, mutex_unlock() error:");
...
/*** TO BE DONE 6.0 START ***/
	...
	if( gmtime_r( &now_t , &now_tm) == NULL )
		fail_errno("In send_response function, gmtime(now) error :");
...
/*** TO BE DONE 6.0 START ***/
		...
		if ( stat(HTML_404, &stat_buffer) )
			fail_errno("In send_response function, stat(HTML_404) error");
...
/*** TO BE DONE 6.0 START ***/
			...
			if ( stat(HTML_501, &stat_buffer) )
				fail_errno("In send_response function, stat(HTML_501) error");
			file_size = stat_buffer.st_size;
...
/*** TO BE DONE 6.0 START ***/
						...
						char set_cookie[100];
						snprintf(set_cookie, 100, "\r\nSet-Cookie: UserID=%d%s", cookie,COOKIE_EXPIRE);
						strcat(http_header, set_cookie);
...
/*** TO BE DONE 6.0 START ***/
	...
	if( gmtime_r( &file_modification_time , &file_modification_tm) == NULL )
		fail_errno("In send_response function, gmtime(file_modification_tm) error :");
...
//la funzione sendfile trasferisce dati tra due fd
 //è più efficace della combinazione read-write poiché, anziché utilizzare lo spazio utente, avviene tutto all'interno del kernel
 if( sendfile( client_fd, fd, 0 , file_size ) < 0 )
 		fail_errno("In send_response function, sendfile() error");
	//non controlliamo quanti byte inviamo
	if ( close(fd) < 0 )
		fail_errno("In send_response function, close() error");
/*** TO BE DONE 6.0 END ***/
...
/*** TO BE DONE 6.0 START ***/
		...
		//reentrant version of strtok(): maintain context between successive calls that parse the same string.
	
		method_str = strtok_r( http_request_line, " " , &strtokr_save);
	//se la stringa da processare è sempre http_request_line inseriamo NULL nelle chiamate a strtok successive	
...
/*** TO BE DONE 6.0 START ***/	
		 		//Il Cookie sarà nella forma: "Cookie: UserID=value", devo isolare value
		 		//manca controllo sulla sintassi dei cooke e sulla strtok
...
/*** TO BE DONE 6.0 END ***/
				 ...
				 *** and possibly add METHOD_CONDITIONAL flag to http_method
                                 ...
                                 ***/
/*** TO BE DONE 6.0 START ***/
			 		//manca controllo sul ritorno di strtok e strptime		
...
/*** TO BE DONE 6.0 START ***/
				...
				if( stat_p->st_mtime >= my_timegm(&since_tm) )
