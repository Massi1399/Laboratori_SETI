...
/*** TO BE DONE 6.0 START ***/
	...
	//questa funzione si occupa di creazione, bind e ascolto sul listen_fd
	create_listening_socket(port_as_str);
	//nel caso fosse andato male qualcosa nella funzione precedente saremo già usciti 	
	drop_privileges();
...
/*** TO BE DONE 6.0 START ***/
		...
		if( pthread_create( &thread_ids[i], NULL , client_connection_thread, &connection_no[i]) != 0)
			fail_errno("In run_webserver function, pthread_create() error:");
...
int main(int argc, char **argv)
{
	...
	www_root = argv[1];
//le pipe vanno create prima della fork per avere i FD condivisi	
	if (pipe(p_to_file))
	...
	if (pid == 0)
	//il figlio esegue il comando file -i e comunica con la funzione webserver tramite le pipe
		run_file(p_to_file, p_from_file);
            else
        //il parent avvia il webserver cominciando dalla chroot         
